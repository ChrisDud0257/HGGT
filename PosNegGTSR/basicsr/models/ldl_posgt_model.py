...
class LDL_PosGT_Model(BaseModel):
    """Base SR model for single image super-resolution."""

    def __init__(self, opt):
        super(LDL_PosGT_Model, self).__init__(opt)
    ...
    def optimize_parameters(self, current_iter):
        ...
        if not self.use_network_d:
            ...
            if (current_iter % self.net_d_iters == 0 and current_iter > self.net_d_init_iters):
                ...
                if self.cri_ldl:
                    self.random_ldl_gt = self.random_l1_gt.clone()
                    pixel_weight = get_refined_artifact_map(self.random_ldl_gt, self.output, self.output_ema, 7)
                    l_g_ldl = self.cri_ldl(torch.mul(pixel_weight, self.output), torch.mul(pixel_weight, self.random_l1_gt))
                    self.l_g_total += l_g_ldl
                    self.loss_dict['l_g_ldl'] = l_g_ldl
