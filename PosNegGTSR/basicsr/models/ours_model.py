...
class Ours_Model(BaseModel):
    ...
    def __init__(self, opt):
        super(Ours_Model, self).__init__(opt)
    ...
    def init_training_settings(self):
        ...
        # define losses
        if train_opt.get('pixel_opt_o'):
            self.cri_pix_o = build_loss(train_opt['pixel_opt_o']).to(self.device)
        else:
            self.cri_pix_o = None
    ...
    def optimize_parameters(self, current_iter):
        ...
        if not self.use_network_d:
            l_pix_o = self.cri_pix_o(self.output, self.l1_gt)

            l_g_total += l_pix_o
            loss_dict['l_p_pix'] = l_pix_o
