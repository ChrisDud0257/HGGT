...
class AdaTarget_PosGT_Model(BaseModel):
    """Base SR model for single image super-resolution."""

    def __init__(self, opt):
        super(AdaTarget_PosGT_Model, self).__init__(opt)
    ...
    def train_net_g(self):
        ...
        _, _, H, W = self.random_l1_gt.size()
        unfold_H = F.unfold(F.pad(self.random_l1_gt, [ds // 2, ds // 2, ds // 2, ds // 2], mode='reflect'),
                            self.s_size, dilation=1,
                            padding=0, stride=self.k_size)  # ([B, 363, N])
        B, _, N = unfold_H.size()
        unfold_H = unfold_H.permute(0, 2, 1).reshape(B * N, -1)

        unfold_S = F.unfold(self.output, self.k_size, dilation=1, padding=0,
                            stride=self.k_size)  # B, C*k_size*k_size, L
        B, _, N = unfold_S.size()
        ...
        transformed_S = F.grid_sample(unfold_S.reshape(-1, 3, self.k_size, self.k_size), grid,
                                      padding_mode='border',
                                      align_corners=None)
        ...
        l_b_pix_total = self.cri_adatarget(transformed_S, self.random_l1_gt)

        self.l_g_total += l_b_pix_total
        self.loss_dict['l_adatarget'] = l_b_pix_total
