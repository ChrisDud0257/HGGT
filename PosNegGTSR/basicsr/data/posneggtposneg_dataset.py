...
class PosNegGTPosNeg_Dataset(data.Dataset):
    """
    ...
    def __init__(self, opt):
        super(PosNegGTPosNeg_Dataset, self).__init__()
        self.opt = opt
        ...
        # Filter out the sets which only have similar GT.
        ...
        # Get the final effective sets, the four images of which has at least have one effective P label or one effective N label
    ...
    def filter_no_effective_set(self, all_json_folder):
        ...
        for json_file in os.listdir(Apath):
            ...
            _, img1_s_num, _ = check_list(*img1_label)
            _, img2_s_num, _ = check_list(*img2_label)
            _, img3_s_num, _ = check_list(*img3_label)
            _, img4_s_num, _ = check_list(*img4_label)

            # s_num >= 2 or the three labels from three people are different means the final label of the image would be S, when three different people give labels to the same image.
            if (img1_s_num >= 2 or len(img1_label) == len(set(img1_label))) \
                    and (img2_s_num >= 2 or len(img2_label) == len(set(img2_label))) \
                    and (img3_s_num >= 2 or len(img3_label) == len(set(img3_label))) \
                    and (img4_s_num >= 2 or len(img4_label) == len(set(img4_label))) :
